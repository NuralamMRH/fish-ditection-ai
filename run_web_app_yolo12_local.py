...
# Initialize Local YOLOv12 detector
try:
    yolo_detector = LocalYOLOv12Fish(confidence=0.3)
    print("✅ Local YOLOv12 detector loaded")
except Exception as e:
    print(f"❌ Failed to load Local YOLOv12: {e}")
    yolo_detector = None

def process_image_with_local_yolo12(image_path):
    """Process image using Local YOLOv12 model and classification."""
    try:
        # Check if detector is available
        if yolo_detector is None:
            return {"error": "Local YOLOv12 detector not available"}
        ...
        for i, fish in enumerate(detections[0]):
            ...
            crop_path = f'temp_local_yolo12_fish_{i}.jpg'
        ...
        annotated_image = create_local_yolo12_annotated_image(image_path, result)
        if annotated_image:
        ...
        return {"error": f"Local YOLOv12 processing failed: {str(e)}"}
...
def create_local_yolo12_annotated_image(image_path, result):
    """Create annotated image with Local YOLOv12 detections."""
    try:
        image = cv2.imread(image_path)
        if image is None:
            return None
        ...
        for i, fish in enumerate(result.get("fish", [])):
            ...
            # Local YOLOv12 detection label
            yolo_label = f"Local-YOLOv12: {fish.get('yolo_detection', 'Unknown')}"
        ...
        filename = f"local_yolo12_annotated_{uuid.uuid4().hex[:8]}.jpg"
...
# HTML Template for Local YOLOv12 app
HTML_TEMPLATE = '''
<!DOCTYPE html>
<html>
<head>
    <title>🐟 Local YOLOv12 Fish Detection</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <style>
        body { 
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            margin: 0; padding: 20px; background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);
        ...
        .model-badge { 
            display: inline-block; background: #00ff88; color: white; 
            padding: 5px 15px; border-radius: 20px; font-size: 12px; font-weight: bold;
        }
        .performance-badge {
            display: inline-block; background: #ff6b9d; color: white;
            padding: 5px 15px; border-radius: 20px; font-size: 12px; font-weight: bold;
            margin-left: 10px;
        }
...
<body>
    <div class="container">
        <div class="header">
            <h1>🐟 Local YOLOv12 Fish Detection System</h1>
            <span class="model-badge">Your Trained Model</span>
            <span class="performance-badge">90%+ Accuracy</span>
            <p>Lightning-fast local processing with your custom-trained model</p>
        </div>
        ...
        <div class="loading" id="loading">
            ...
            <p>🚀 Processing with your trained YOLOv12...</p>
        </div>
    ...
    <script>
        ...
        function displayResults(data) {
            ...
            html += '<h3>🎯 Local YOLOv12 Detection Results</h3>';
            html += '<p><strong>Model:</strong> ' + (data.model || 'YOLOv12-Local-Trained') + ' ⚡</p>';
            ...
            data.fish.forEach((fish, index) => {
                ...
                html += '<p><strong>Local YOLOv12:</strong> ' + fish.yolo_detection + 
...
def api():
    try:
        ...
        # Process with Local YOLOv12
        result = process_image_with_local_yolo12(upload_path)
...
if __name__ == '__main__':
    print("🐟 Starting Local YOLOv12 Fish Detection System...")
    print("🔍 Testing Local YOLOv12 models...")
    
    # Test Local YOLOv12
    if yolo_detector:
        print("✅ Local YOLOv12 Detection: Working")
        yolo_ok = True
    else:
        print("❌ Local YOLOv12 Detection: Error")
        yolo_ok = False
...
"""
    ...
    if yolo_ok and class_ok:
        print("\n🎉 All Local YOLOv12 systems working!")
    else:
    ...
    print("\n🚀 Starting Local YOLOv12 server...")
    print("📱 Open: http://localhost:5003")
    print("✨ Features: Local YOLOv12 + Enhanced Classification")
